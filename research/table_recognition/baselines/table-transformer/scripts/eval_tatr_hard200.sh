...
MODEL_LOAD_PATH="${1:-${MODEL_LOAD_PATH:-${DATA_ROOT}/output/tatr_no_graph_from_scratch/model_best.pth}}"
...
DEBUG_SAVE_DIR="${DEBUG_SAVE_DIR:-${DATA_ROOT}/output/tatr_eval_debug}"
DEBUG="${DEBUG:-0}"

CHECKPOINT_DIR="$(dirname "${MODEL_LOAD_PATH}")"
LOG_DIR="${LOG_DIR:-${CHECKPOINT_DIR}/logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/eval_tatr_$(date +%Y%m%d_%H%M%S).log}"
mkdir -p "${LOG_DIR}"
