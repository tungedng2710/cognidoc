"""
Training and evaluation entrypoint for AuxGraphTATR.

Run from this directory, mirroring the original Table Transformer scripts:
    python train_aux_graph_tatr.py --data_type structure --config_file aux_graph_tatr_config.json --data_root_dir /path/to/data

The deployed/evaluated model remains image-only. Targets are passed into the
model only during training to activate the auxiliary graph branch.
"""
import argparse
import json
import math
...
def get_args():
    parser = argparse.ArgumentParser("AuxGraphTATR training/evaluation")
    ...
    parser.add_argument("--model_load_path", default=None, help="Baseline or AuxGraphTATR checkpoint path")
    ...
    parser.add_argument("--train_strategy", choices=["full", "decoder_aux", "aux_only"], default=None)
...
def load_model_checkpoint(model, model_load_path, device):
    ...
    return checkpoint, has_aux_graph_keys
...
def train_one_epoch_aux_graph(
    ...
    metric_logger.add_meter("graph_relation_error", utils.SmoothedValue(window_size=1, fmt="{value:.2f}"))
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        baseline_loss_dict = criterion(outputs, targets)
        aux_loss_dict = model.compute_auxiliary_losses(outputs, targets, criterion.matcher)
        weight_dict = criterion.weight_dict

        baseline_losses = weighted_loss(baseline_loss_dict, weight_dict)
        graph_loss = aux_loss_dict["loss_graph_aux"] * lambda_graph
        distill_loss = aux_loss_dict["loss_distill"] * lambda_distill
        losses = baseline_losses + graph_loss + distill_loss

        baseline_loss_reduced = utils.reduce_dict(baseline_loss_dict)
        aux_loss_reduced = utils.reduce_dict(aux_loss_dict)
        baseline_loss_scaled = {
            k: v * weight_dict[k]
            for k, v in baseline_loss_reduced.items()
            if k in weight_dict
        }
        baseline_loss_unscaled = {f"{k}_unscaled": v for k, v in baseline_loss_reduced.items()}
        aux_loss_scaled = {
            "loss_graph_aux": aux_loss_reduced["loss_graph_aux"] * lambda_graph,
            "loss_distill": aux_loss_reduced["loss_distill"] * lambda_distill,
        }
        total_loss_reduced = sum(baseline_loss_scaled.values()) + sum(aux_loss_scaled.values())

        loss_value = total_loss_reduced.item()
        if not math.isfinite(loss_value):
            print("Loss is {}, stopping training".format(loss_value))
            print(baseline_loss_reduced)
            print(aux_loss_reduced)
            sys.exit(1)
        ...
        metric_logger.update(
            loss=loss_value,
            **baseline_loss_scaled,
            **baseline_loss_unscaled,
            **aux_loss_scaled,
            loss_graph_aux_unscaled=aux_loss_reduced["loss_graph_aux"],
            loss_distill_unscaled=aux_loss_reduced["loss_distill"],
        )
        metric_logger.update(class_error=baseline_loss_reduced["class_error"])
        metric_logger.update(graph_relation_error=aux_loss_reduced["graph_relation_error"])
...
def train(args, model, criterion, postprocessors, device, loaded_checkpoint=None, loaded_aux_checkpoint=False):
    ...
    if (
        ...
        loaded_checkpoint is not None
        and not args.load_weights_only
        and not loaded_aux_checkpoint
        and isinstance(loaded_checkpoint, dict)
        and "optimizer_state_dict" in loaded_checkpoint
    ):
        print("Ignoring baseline optimizer state because it does not match AuxGraphTATR parameters.")
    ...
    if args.model_save_dir:
        ...
        output_directory = os.path.join(args.data_root_dir, "output", "aux_graph_tatr_" + run_date)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_aux_graph(
            model,
            criterion,
            data_loader_train,
            optimizer,
...
def main():
    ...
    print("loading AuxGraphTATR model")
    model, criterion, postprocessors = build_aux_graph_model(args)
    model.to(device)

    loaded_checkpoint = None
    loaded_aux_checkpoint = False
    if args.model_load_path:
        loaded_checkpoint, loaded_aux_checkpoint = load_model_checkpoint(model, args.model_load_path, device)
    ...
    if args.mode == "train":
        train(
            ...
            loaded_aux_checkpoint=loaded_aux_checkpoint,
        )
    elif args.mode == "eval":
